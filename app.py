...
def compute_indicators(df):
    ...
    df["EMA_20"]  = ta_lib.ema(c, length=20)
    df["EMA_50"]  = ta_lib.ema(c, length=50)
    df["EMA_200"] = ta_lib.ema(c, length=200)
    df["SMA_20"]  = ta_lib.sma(c, length=20)

    rsi = ta_lib.rsi(c, length=14)
    df["RSI"] = rsi

    macd_df = ta_lib.macd(c)
    if macd_df is not None:
        df["MACD"]        = macd_df.iloc[:, 0]
        df["MACD_Signal"] = macd_df.iloc[:, 2]
        df["MACD_Hist"]   = macd_df.iloc[:, 1]

    bb = ta_lib.bbands(c, length=20)
    if bb is not None:
        df["BB_Upper"] = bb.iloc[:, 2]
        df["BB_Mid"]   = bb.iloc[:, 1]
        df["BB_Lower"] = bb.iloc[:, 0]

    df["ATR"]  = ta_lib.atr(h, l, c, length=14)
    df["OBV"]  = ta_lib.obv(c, v)
...
# ─── Main Dashboard ───────────────────────────────────────────────────────────
st.markdown("# StockSense AI &nbsp; <span style='font-size:1rem;color:#8b949e;font-weight:400'>Powered by XGBoost + Technical Analysis</span>", unsafe_allow_html=True)
